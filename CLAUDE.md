...
2. **Chat** (centre): one headless `claude` session per project. Claude checks and edits the
...
npm run build          # out/
npm run dist           # AppImage + deb in dist/ (then scripts/install-desktop.sh for the icon)
...
- `src/main/` — Electron main:
  ...
  - `claudeChat.ts` — port of conductor-linux's chat (no subagents/workflows): spawns
    ...
    picked in the composer and applied by restarting the session with `--model/--effort` (persisted on the project).
