...
/**
 * One project card — the same shape as a workspace row in conductor-linux's sidebar: a padded
 * plate whose active state is a green line along its left edge, the name on top with the status
 * indicators right-aligned, and a dim meta line (the last check's counts) underneath.
 */
function ProjectRow({ p, active, busy, onSelect, onDelete }: { p: PcbProject; active: boolean; busy: boolean; onSelect: () => void; onDelete: () => void }): JSX.Element {
  ...
  return (
    <div className={'project-item' + (active ? ' active' : '')} onClick={onSelect} role="button" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && onSelect()} title={p.dir}>
      <div className="project-top">
        <span className="project-name">{p.name}</span>
        <span className="project-indicators">
          {busy && <span className="busy-dot on" title="Claude працює" />}
