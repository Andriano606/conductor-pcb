...
- **Чат** (у центрі): окрема сесія `claude` на кожен проект. Ви кажете, що змінити, Claude
  ...
  У полі вводу: слеш-команди, історія стрілками, вкладення, вибір моделі й зусиль, бібліотека
  промтів (📖, змінні `$PCB_PROJECT_NAME`, `$PCB_PROJECT_DIR`, `$PCB_BOARD_FILE`) і конфігурації Claude
  (⚙: папки зі скілами, командами, CLAUDE.md, які накладаються на `~/.claude` для чату цього проекту).
- **Ліміти** (внизу ліворуч): результат `/usage` — сесія і тиждень, оновлюється кожні 30 с.
...
попросити Claude написати його: увімкніть конфіг `kicad-pcb-rules` (скіл pcb-rule-author) з `~/Documents/claude-configs`.
